...
관계형 DB. 스토리지 오토 스케일링 기능 제공. 3가지 스토리지 유형이 있음. 프로비저닝된 IOPS SSD 스토리지, 범용SSD,  마그네틱 스토리지. 언급된 순서대로 저렴하고 늦어짐. 

#### 백업

자동 백업이 가능.  자동 백업을 할 경우 1일에서 최대 35일까지 보존 가능하고 기본적으로 5분 단위로 보존.

스냅샷을 남겨서 수동 백업이 가능하고 스냅샷은 보존 기간이 없음. S3에 저장

#### 보안

KMS를 이용해 DB엔진에 대한 저장 중 암호화 가능

SSL/TLS를 이용해 인스턴스 간 전송 중 암호화 가능

IAM 사용자 및 역할을 통해 DB인증 적용 가능

보안 그룹을 연결해 IP 주소, EC2 인스턴스에 대한 DB 연결 제어

RDS Audit Logs 가능을 사용해 보안 감사에 활용 가능

#### Read Replica

읽기 전용으로 가능한 인스턴스 여러개 만들어서 트래픽 분산이 가능하다.  

#### RDS Custom

사용자가 OS와 DB를 모두 관리 권한을 갖게 하기 위해 필요. 

#### RDS proxy

클라이언트와 DB 사이에 연결 pooling만 관리. 

1. 장애 조치 시간 감소 => 예비 DB인스턴스에 연결만 하면 되기 때문.
2. 보안 개선 => DB에 AWS IAM인증을 적용하고 AWS Secrets Manager에 보안 인증 정보를 안전하게 저장
3. 데이터 베이스 성능 유지 => 클라이언트가 DB연결 설정하는 메모리 CPU 부하를 감소시켜줄 수 있다. 또 DB 연결 수를 쉽게 제한할 수 있다. 

### 2. Aurora

> RDS 호환형 관계형 데이터 베이스. RDS에서 제공하는 읽기 전용 복제본, KMS암호화, 스냅샷 백업, 오토스케일링, RDS Proxy 등을 제공. AWS에서 만든 서비스로 다른 RDS보다 저렴한 비용에 성능이 더 뛰어남. 다른 RDS보다 속도는 3-5배 빠름. 데이터베이스 설정, 패치 적용 및 백업과 같은 관리 태스크를 자동화. 개별 DB 인스턴스 기반이 아닌 여러 인스턴스를 하나로 운영하는 클러스터 DB 기반으로 구성 됨
...
인메모리 데이터 스토어. 빠른 응답이 필요한 대플리케이션에 사용. 실제 DB앞에 위치. 세션, 게임 리더보드, 스트리밍 및 분석과 같이 내수성이 필요하지 않는 기본 데이터 스토어로 사용. `Memcached`는 멀티쓰레드 지원, Redis는 싱글쓰레드만 지원. 
...
- 쇼핑몰 장바구니, 은행 트랜잭션, 게임 플레이어 기록저장 등에 사용 
- 백업 및 복구 가능
...
특정기간동안 사용하지 않은 경우 삭제

#### 글로벌 테이블

모든 리전에서 읽기 쓰기 가능. 리전 간 DB를 복제하는 기능

#### Dynamo Streams

변경사항이 있을 때 변경사항을 캡쳐하고 `kinesis Data Stream`으로 보내서 Lamda로 트리거하여 Amazon SNS로 전송해 이벤트를 발생시킬 수 있음.

#### 읽기 일관성

- 최종적 일관된 읽기 : 읽기 처리량 최대화를 목표. 모든 변경 사항을 읽지 못 할 수 있음 
- 강력한 일관된 읽기 : 읽기 전 성공적인 응답을 수신한 모든 쓰기를 반영한 결과를 반영. 처리 비용 증가

#### S3와 통합

테이터를 S3 버킷으로 내보내기가 가능. 다른 리전으로 보낼 수 있고 `JSON` 또는 `Amazon Ion`형식으로 보낼 수 있음.

 ### 4. Database Migration Service

1. 복제 인스턴스를 먼저 생성해줘야 한다.

2. 엔드포인트 생성해준다.
3. 데이터 마이그레이션 태스트를 생성해준다.
...
표준 SQL을 사용해  S3에 저장된 데이터를 분석할 수 잇는 쿼리 서비스. CSV, JSON 등 다양한 종류의 데이터 형식을 지원. athena로 데이터 로드 필요 없이 S3에 저장된 데이터를 직접 사용. `Amazon QuickSight`와 통합해서 쿼리된 데이터를 시각화 할 수 있음.

### Redshift

 데이터 웨어하우스 서비스. 아마존 서비스와 SSH지원 가능한 모든 온프레미스 호스트, 데이터 소스에서 데이터 로드 가능.
...
#### 제한시간 초과/표시제한 시간
 ...
 메세지 소비자가 대기열에서 메시지를 수신하고 처리하면 메시지는 계속 대기열에 있음
